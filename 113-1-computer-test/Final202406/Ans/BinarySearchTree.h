...
bool BinarySearchTree<ItemType>::isNonLeafNode_Recu(BinaryNode<ItemType> *subtreePtr, const ItemType target)
{
    ...
    if (target > subtreePtr->getItem())
    {
        return isNonLeafNode_Recu(subtreePtr->getRightChildPtr(), target);
    }
    else if (target < subtreePtr->getItem())
    {
        return isNonLeafNode_Recu(subtreePtr->getLeftChildPtr(), target);
    }
    else
    {
        if (!subtreePtr->isLeaf())
...
int BinarySearchTree<ItemType>::nonLeafCount_Iter(BinaryNode<ItemType> *subtreePtr)
{
    ...
    queue.push(subtreePtr);

    while (subtreePtr && !queue.empty())
    {
        BinaryNode<ItemType> *temp = queue.front();
        queue.pop();
        ...
        if (temp->getLeftChildPtr())
            queue.push(temp->getLeftChildPtr());

        if (temp->getRightChildPtr())
            queue.push(temp->getRightChildPtr());
    }
...
{
    if (!subtreePtr || (!subtreePtr->getLeftChildPtr() && !subtreePtr->getRightChildPtr()))
        return 0;
...
bool BinarySearchTree<ItemType>::isBalancedHelper(BinaryNode<ItemType> *subTreePtr)
{
    ...
    if (abs(leftHeight - rightHeight) <= 1 &&
        isBalancedHelper(subTreePtr->getLeftChildPtr()) &&
        isBalancedHelper(subTreePtr->getRightChildPtr()))
        return true;
