...
BinaryNode<string> pqArrayMaxWord(BinaryNode<string> *pqArray, int &size)
{
    ...
    {
        if (maxCount < pqArray[i].getCount())
        {
            maxCount = pqArray[i].getCount();
            entry = pqArray[i];
        }
    }
...
int pqArrayWordCount(BinaryNode<string> *pqArray, int &size, int index)
{
    ...
    return pqArray[index].getCount() + pqArrayWordCount(pqArray, size, index + 1);
}
...
int RemoveMaxEdges(int **matrixGraph, int numVertices)
{
    ...
    for (size_t i = 0; i < numVertices; i++)
    {
        ...
        {
            if (matrixGraph[i][j] != INT_MAX && maxWeight < matrixGraph[i][j])
                maxWeight = matrixGraph[i][j];
        }
    }
