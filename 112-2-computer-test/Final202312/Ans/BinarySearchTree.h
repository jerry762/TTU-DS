...
protected:
   ...
   int leafCount_Iter(BinaryNode<ItemType>* subTreePtr);
   int leafCount_Recu(BinaryNode<ItemType>* subTreePtr);
...
void BinarySearchTree<ItemType>::levelorder(void visit(BinaryNode<ItemType>&), BinaryNode<ItemType>* subtreePtr) const
{
	...
	{
		BinaryNode<ItemType>* currNode = nodeQueue.front();

		visit(*currNode);

		nodeQueue.pop();

		if (currNode->getLeftChildPtr())
			nodeQueue.push(currNode->getLeftChildPtr());

		if (currNode->getRightChildPtr())
			nodeQueue.push(currNode->getRightChildPtr());
	}
}  // end levelorder
...
bool BinarySearchTree<ItemType>::isLeafNode_Iter(BinaryNode<ItemType>* subtreePtr, const ItemType target)
{
	...
	nodeStack.push(subtreePtr);

	while (!nodeStack.empty())
	{
		BinaryNode<ItemType>* top = nodeStack.top();

		if (top->getItem() == target && !top->getLeftChildPtr() && !top->getRightChildPtr())
			return true;

		nodeStack.pop();

		if (top->getRightChildPtr())
			nodeStack.push(top->getRightChildPtr());

		if (top->getLeftChildPtr())
			nodeStack.push(top->getLeftChildPtr());
	}
...
bool BinarySearchTree<ItemType>::isLeafNode_Recu(BinaryNode<ItemType>* subtreePtr, const ItemType target)
{
	if (subtreePtr)
	{
		if (subtreePtr->getItem() == target && !subtreePtr->getLeftChildPtr() && !subtreePtr->getRightChildPtr())
			return true;

		return isLeafNode_Recu(subtreePtr->getLeftChildPtr(), target) || isLeafNode_Recu(subtreePtr->getRightChildPtr(), target);
	}
...
template<class ItemType>
int BinarySearchTree<ItemType>::countLeaves(string str)
{
	if ( str == "i")
		return leafCount_Iter(rootPtr);
	if (str == "r")
		return leafCount_Recu(rootPtr);

	return 0;
}

template<class ItemType>
int BinarySearchTree<ItemType>::leafCount_Iter(BinaryNode<ItemType>* subtreePtr)
{
	stack<BinaryNode<ItemType>*> nodeStack;
	int count = 0;
	...
	while (!nodeStack.empty())
	{
		...
		if (!top->getLeftChildPtr() && !top->getRightChildPtr())
			count++;
...
template<class ItemType>
int BinarySearchTree<ItemType>::leafCount_Recu(BinaryNode<ItemType>* subtreePtr)
{
	if (subtreePtr)
	{
		...
		return leafCount_Recu(subtreePtr->getLeftChildPtr()) + leafCount_Recu(subtreePtr->getRightChildPtr());
	}
	return 0;
}
#endif
